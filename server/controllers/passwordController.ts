...
export default class passwordController {
  static async myPassword ( req:Request, res:Response, next:NextFunction ) {
    try{
      ...
      const data: IPassword[] = await Password.find({ UserId:_id }).populate('UserId')
  ...
  static async findOnePassword ( req:Request, res:Response, next:NextFunction ) {
    try{
      const { _id } = req.params // password Id
      const data: IPassword | null = await Password.findOne({ _id }).populate('UserId')
      if(data){
        res.status(200).json(data)
      }
      else{
  ...
  static async createPassword ( req:Request, res:Response, next:NextFunction ) {
    try{
      ...
      const data = await Password.create({  UserId:_id, passwordData, nameData, urlData })
      res.status(200).json(data)
    }
    catch(err){
      next(err)
  ...
  static async updatePassword ( req:Request, res:Response, next:NextFunction ) {
    try{
      ...
      const { _id } = req.params // password id
      const { passwordData, nameData, urlData } = req.body
      const data: IPassword | null = await Password.findOneAndUpdate({ _id },{ passwordData, nameData, urlData },{ new: true })
      if(data){
        res.status(200).json(data)
      }
      else{
